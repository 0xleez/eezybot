...
class WalletsChecker {
  ...
  static async checkBlocks() {
    ...
    const startBlock = endBlock - 5000
    ...
    for (let [name, account] of Object.entries(accounts)) {
      ...
      for (let tx of transactions) {
        ...
        if (!hashes.includes(tx.hash)) {
          await WalletsChecker.sendDiscordMessage(`A new txn on ${name}'s wallet! https://etherscan.io/tx/${tx.hash} https://debank.com/profile/${account}/history`)
